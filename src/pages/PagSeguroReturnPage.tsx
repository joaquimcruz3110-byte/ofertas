...
const PagSeguroReturnPage = () => {
  ...
  useEffect(() => {
    const processPaymentStatus = async () => {
      ...
      const toastId = showLoading('Verificando pagamento PagSeguro...');

      try {
        // Em um cenário real, você faria uma chamada à API do PagSeguro para verificar o status
        // usando o transactionId. Por simplicidade, vamos consultar nosso próprio banco de dados.
        const { data: salesData, error: salesError } = await supabase
          .from('sales')
          .select('payment_gateway_status')
          .eq('payment_gateway_id', referenceId) // Usamos nosso referenceId para buscar
          .eq('buyer_id', buyerId)
          .limit(1)
          .single();
        ...
        if (salesData.payment_gateway_status === 'completed') {
          ...
          setMessage('Pagamento pendente. Aguardando confirmação do PagSeguro. Você será redirecionado(a) quando for confirmado.');
...
export default PagSeguroReturnPage;