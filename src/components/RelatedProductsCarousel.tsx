...
const AUTOPLAY_INTERVAL = 3200; // 3.2 segundos

const RelatedProductsCarousel = ({ currentProductId, currentProductCategory }: RelatedProductsCarouselProps) => {
  ...
  const fetchRelatedProducts = useCallback(async () => {
    ...
    let query = supabase
      .from('products')
      .select('id, name, price, photo_urls, shop_details(shop_name)')
    ...
    if (currentProductCategory) {
      query = query.eq('category', currentProductCategory);
    } else {
      // Se não houver categoria, buscar produtos aleatórios ou mais recentes
    ...
    if (error) {
      showError('Erro ao carregar produtos relacionados: ' + error.message);
      console.error('Erro ao carregar produtos relacionados:', error.message);
      setProducts([]);
    } else {
